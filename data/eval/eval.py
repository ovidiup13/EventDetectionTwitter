...
def main():
    ...
    eventsFilePath = sys.argv[2]
    eventsDescPath = sys.argv[3]
    eventsCategoriesPath = sys.argv[4]
    
    rels_file = open(eventsFilePath, 'r')
    ...
    desc_file = open(eventsDescPath, 'r')
    for line in desc_file:
        event, desc = line.strip().split(" ", 1)
        descriptions[event] = desc[1:-1].strip()

    cat_file = open(eventsCategoriesPath, 'r')
    ...
    if valid_count > 0 and len(set(above_threshold)) > 0 and len(set(found_above_threshold)) > 0:
        ...
        print "model.Cluster Precision:\t%1.3f" % (event_precision)
